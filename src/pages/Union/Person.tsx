...
const PersonPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!unionId) {
      navigate("/union"); // Redirect to union list instead of home
      return;
    }

    const unionRef = ref(db, `union/${unionId}`);

    try {
      onValue(unionRef, (snapshot) => {
        const data = snapshot.val();
        if (data) {
          setUnionName(data.name || "");
          ...
          setError("ইউনিয়ন খুঁজে পাওয়া যায়নি");
          setTimeout(() => navigate("/union"), 3000); // Redirect after 3 seconds
        }
        setLoading(false);
      });
    } catch (err) {
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1200, mx: "auto" }}>
      ...
      >
        ...
        {unionName || "ইউনিয়ন সদস্যদের তালিকা"}
      </Typography>
