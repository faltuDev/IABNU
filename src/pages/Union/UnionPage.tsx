...
const UnionPage: React.FC = () => {
  ...
  useEffect(() => {
    const unionsRef = ref(db, 'union');
    
    try {
      onValue(unionsRef, (snapshot) => {
        const data = snapshot.val();
        if (data) {
          const unionsArray = Object.keys(data).map((key) => ({
            id: key,
            name: data[key].name
          }));
  ...
  const handleUnionClick = (unionId: string) => {
    navigate(`/union/${unionId}/person`);
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center mb-8 space-y-2">
        ...
        <p className="text-gray-500 text-center">আপনার ইউনিয়ন নির্বাচন করুন</p>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {unions.map((union, index) => (
            <motion.div
              ...
              onClick={() => handleUnionClick(union.id)}
              role="button"
              tabIndex={0}
            >
